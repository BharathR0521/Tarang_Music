...
export default function Songs() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 pt-10 pb-32">
      <div className="flex flex-wrap items-end justify-between gap-4 mb-8">
        ...
        {user && <button onClick={() => setShowUpload(true)} className="inline-flex items-center gap-2 bg-amber text-base font-medium rounded-full px-4 py-2 hover:brightness-110 transition"><FiUpload size={16} /> Upload song</button>}
      </div>
      {error && <p className="mb-5 text-sm text-red-300">{error}</p>}
      {loading ? <p className="text-sm text-muted">Loading your library...</p> : songs.length === 0 ? (
        <div className="empty-state"><FiMusic size={28} className="text-teal" /><p className="text-ink mt-3">Your library is quiet.</p><p className="text-sm text-muted mt-1">Upload a song to start building it.</p></div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
          {songs.map((song) => <SongCard key={song._id} song={song} songList={songs} liked={user && song.likes?.includes(user._id)} onLike={handleLike} onAddToPlaylist={user ? setAddingSong : null} onDelete={user && song.uploadedBy === user._id ? handleDelete : null} />)}
        </div>
      )}
