...
export default function Home() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-5 pt-8 pb-32">
      <section className="hero-panel mb-12">
        <div className="hero-copy">
          ...
          <Link to="/songs" className="inline-flex items-center gap-2 bg-amber text-base font-medium rounded-full px-4 py-2 mt-7 hover:brightness-110 transition">Explore songs <FiArrowUpRight size={16} /></Link>
        </div>
      ...
      {user && !loading && displayedRecentSongs.length > 0 && (
        <section>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">{displayedRecentSongs.map((song) => <SongCard key={song._id} song={song} songList={displayedRecentSongs} />)}</div>
        </section>
      )}
    </div>
  );
